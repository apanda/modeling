...
import z3
def TrivialSynth ():
    ctx = components.Context(['a', 'b', 'c', 'f', 'd'], \
                            ['ip_a', 'ip_b', 'ip_c', 'ip_f', 'ip_d'])
    net = components.Network(ctx)
    dpi_policy = components.DPIPolicy('hate_bad')
    dpi_policy2 = components.DPIPolicy('hate_bad2')
    ctx.AddPolicy(dpi_policy)
    ctx.AddPolicy(dpi_policy2)
    a = components.EndHost(ctx.a, net, ctx) 
    b = components.EndHost(ctx.b, net, ctx) 
    c = components.EndHost(ctx.c, net, ctx) 
    #f = components.AclFirewall(ctx.f, net, ctx)
    f = components.DpiFW(dpi_policy2, ctx.f, net, ctx)
    d = components.DpiFW(dpi_policy, ctx.d, net, ctx)
    net.setAddressMappings([(a, ctx.ip_a), \
                            (b, ctx.ip_b), \
                            (c, ctx.ip_c), \
                            (f, ctx.ip_f), \
                            (d, ctx.ip_d)])
    net.RecoverableRoutingPolicy(a)
    net.RecoverableRoutingPolicy(b)
    net.RecoverableRoutingPolicy(c)
    net.RecoverableRoutingPolicy(f)
    net.RecoverableRoutingPolicy(d)
    ...
    p3 = z3.Const('p3', ctx.packet)
    p4 = z3.Const('p4', ctx.packet)
    const = []
    const.append(ctx.packet.src(p) == ctx.ip_a)
    const.append(ctx.packet.dest(p) == ctx.ip_c)
    const.append(ctx.packet.origin(p) == a.z3Node)
    const.append(dpi_policy.dpi_match(ctx.packet.body(p)))
    const.append(ctx.etime(a.z3Node, p, ctx.send_event) > 0)
    const.append(ctx.etime(c.z3Node, p, ctx.recv_event) == 0)

    const.append(ctx.packet.src(p4) == ctx.ip_a)
    const.append(ctx.packet.dest(p4) == ctx.ip_c)
    const.append(ctx.packet.origin(p4) == a.z3Node)
    const.append(dpi_policy2.dpi_match(ctx.packet.body(p4)))
    const.append(z3.Not(dpi_policy.dpi_match(ctx.packet.body(p4))))
    const.append(ctx.etime(a.z3Node, p4, ctx.send_event) > 0)
    const.append(ctx.etime(c.z3Node, p4, ctx.recv_event) == 0)

    const.append(ctx.packet.src(p3) == ctx.ip_a)
    const.append(ctx.packet.dest(p3) == ctx.ip_c)
    const.append(ctx.packet.origin(p3) == a.z3Node)
    #const.append(dpi_policy.dpi_match(ctx.packet.body(p3)))
    const.append(ctx.etime(a.z3Node, p3, ctx.send_event) > 0)
    const.append(ctx.etime(c.z3Node, p3, ctx.recv_event) > 0)
