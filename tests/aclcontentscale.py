...
for size in xrange(start_size, stop_size):
    for it in xrange(iters):
      ...
      start = time.time()
      res = model.check.CheckDataIsolationProperty(model.endhosts[0], model.endhosts[1])
      stop = time.time()
      unsat_time = stop - start
      start = time.time()
      res = model.check.CheckDataIsolationProperty(model.endhosts[0], model.endhosts[2])
      stop = time.time()
      sat_time = stop - start
