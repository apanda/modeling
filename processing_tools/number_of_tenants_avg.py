...
def Process (fnames):
    ...
    tenant_min = defaultdict(lambda: defaultdict(lambda: float("inf")))
    tenant_max = defaultdict(lambda: defaultdict(lambda:0.0))
    for fname in fnames:
        f = open(fname)
        for l in f:
            ...
            tenant_run[(priv, pub)][tenants] += (int_checks + ext_checks + oext_check)
            tenant_max[(priv, pub)][tenants] = max(tenant_max[(priv, pub)][tenants], total)
            tenant_min[(priv, pub)][tenants] = max(tenant_min[(priv, pub)][tenants], total)

    for k in sorted(tenant_run.keys()):
        print "# ----%s------"%(str(k))
        for k2 in sorted(tenant_run[k].keys()):
            print "%d %d %f %f %f"%(k2, tenant_run[k][k2], \
                    tenant_time[k][k2]/float(tenant_run[k][k2]), \
                    tenant_min[k][k2], tenant_max[k][k2])
        print
        print
